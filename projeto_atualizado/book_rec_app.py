...
class VespaApp:
        ...
        package = ApplicationPackage(
            ...
            schema=[
                Schema(
                    ...
                    document=Document(
                        fields=[
                            ...
                            Field(
                                ...
                                indexing=[
                                    'input title . " " . input authors . " " . input description',
                                    "embed",
                                    "index",
                                    "attribute",
                                ],
                    ...
                    fieldsets=[FieldSet(name="default", fields=["title", "authors", "description"])],
                    rank_profiles=[
                        RankProfile(
                            name="bm25",
                            inputs=[("query(q)", "tensor<float>(x[384])")],
                            functions=[
                                Function(name="bm25sum", expression="bm25(title) + bm25(authors) + bm25(description)")
                            ],
                            first_phase="bm25sum",
                        ),
                        RankProfile(
    ...
    def transform_row(self, row):
        return {
            "id": row["isbn10"],
            "fields": {"title": row["title"], "authors": row["authors"], "description": row["description"] ,"id": row["isbn10"]},
        }
    ...
    def query_bm25(self, query, limit = 10):
        ...
        return self.hits_as_df(response, ['id', 'title', 'authors', 'description'])
